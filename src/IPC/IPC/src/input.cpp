...
void Dynamic_Data_t::feed(ipc::fake_rcConfig &config) {
...
void Goal_Data_t::feed(const geometry_msgs::PoseStampedConstPtr pMsg)
{
    msg = *pMsg;
    rcv_stamp = ros::Time::now();
    recv_new_msg = true;
    ...
    last_goal = new_goal; //DEBUG
}
PointCloud_Data_t::PointCloud_Data_t()
{
    rcv_stamp = ros::Time(0);
    recv_new_msg = false;
}
void PointCloud_Data_t::feed(const sensor_msgs::PointCloud2ConstPtr pMsg)
{
    msg = *pMsg;
    rcv_stamp = ros::Time::now();
    recv_new_msg = true;

...
namespace so3_quadrotor {
    ...
    private:
        ...
        void timer_callback(const ros::TimerEvent &event) {
            ...
            if (tnow >= next_odom_pub_time) {
                ...
                Eigen::Vector3d acc_with_g = acc + g;

                Eigen::Vector3d acc_with_g_b = quat.toRotationMatrix() * acc_with_g;

                imu_msg_.linear_acceleration.x = acc_with_g_b[0];
                imu_msg_.linear_acceleration.y = acc_with_g_b[1];
                imu_msg_.linear_acceleration.z = acc_with_g_b[2];
    ...
    public:
        void onInit(void) {
            ...
            cmd_sub_ = nh.subscribe<quadrotor_msgs::SO3Command>("so3cmd", 10, &Nodelet::cmd_callback, this,

...
int main(int argc, char** argv)
{
  ...
  //rsc process
    ...
    if((file_name.substr(file_name.length()-rsc_highres.length(),file_name.length())).compare(rsc_highres) == 0 ||
     (file_name.substr(file_name.length()-rsc_lowres.length(),file_name.length())).compare(rsc_lowres) == 0)
    {
    for (int i = 0; i < cloud.points.size(); ++i)
    {
      auto pt = cloud.points[i];
  ...
  // Mainbuilding process
    ...
    if((file_name.substr(file_name.length()-Mainbuilding_highres.length(),file_name.length())).compare(Mainbuilding_highres) == 0 ||
     (file_name.substr(file_name.length()-Mainbuilding_lowres.length(),file_name.length())).compare(Mainbuilding_lowres) == 0)
    {
    for (int i = 0; i < cloud.points.size(); ++i)
    {
      auto pt = cloud.points[i];
  ...
  // {
  //   auto pt = cloud.points[i];
  //   if(pt.x > 6 && pt.x <8 && pt.y > -8 && pt.y < -6 && pt.z > 0 && pt.z < 1.8)
  //   {
  ...
  //Yuen Lang 02 process
    ...
    if((file_name.substr(file_name.length()-yuanlang02_highres.length(),file_name.length())).compare(yuanlang02_highres) == 0 || 
    (file_name.substr(file_name.length()-yuanlang02_lowres.length(),file_name.length())).compare(yuanlang02_lowres) == 0)
    {
    for (int i = 0; i < cloud.points.size(); ++i)
    {
      auto pt = cloud.points[i];
  ...
  //Library G process
    ...
    if((file_name.substr(file_name.length()-library_highres.length(),file_name.length())).compare(library_highres) == 0 ||
     (file_name.substr(file_name.length()-library_lowres.length(),file_name.length())).compare(library_lowres) == 0)
    {
    for (int i = 0; i < cloud.points.size(); ++i)
    {
      auto pt = cloud.points[i];
